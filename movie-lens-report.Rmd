...
sample %>%
  ...
  geom_linerange(
    aes(x = timestamp, ymin = pmin(rating, mu),
        ymax = pmax(rating, mu), color = "original error")) +
...
Does this effect achieve significant gains from our previous RMSE?. It does.
...
model_movie_effect <- mu + movie_effect_on_test_set$b_i
...
Notice how the movie mean does indeed change over time. This effect could be explained by seasonality of the movies, or changing taste of users over time. Regardless, it is there, so we modify our model to account for it:
...
print(paste("RMSE of average + temporal movie effect: ", min(binned_movie_effect_rmses)))
```
...
model_movie_and_user_effect <-
    mu +
    user_effect_on_test_set$b_i +
    coalesce(user_effect_on_test_set$b_i_t, 0) +
    user_effect_on_test_set$b_u
...
The final improvement we do in this work is the application of regularization. We want to somehow control the effect of ratings where we have low confidence, that is, movies that were rated by few users, and users that made few ratings should weight less. And we do a significant amount of movies and users in that category:
...
As per (dsbook): "Regularization permits us to penalize large estimates that are formed using small sample sizes... Specifically, instead of minimizing the least squares equation, we minimize an equation that adds a penalty: $$
...
regularized_rmses <- sapply(lambdas, function(lambda) {
  ...
  model_regularized <-
    mu +
    regulariztion_on_test_set$b_i +
    coalesce(regulariztion_on_test_set$b_i_t, 0) +
    regulariztion_on_test_set$b_u

  RMSE(test_set$rating, model_regularized)
})
...
print(paste("RMSE of baseline + temporal movie effect and user effect + regularization: ", min(regularized_rmses)))
