...
$b_i(t)$: term that accounts for the "effect" (or "bias") allocated to the rated item. We notice that our data also has a temporal component, thus we make this term be a function of time.
...
and
$$
\hat{b}_u(\lambda) = \frac{1}{\lambda + n_u} \sum_{i=1}^{n_u} \left(Y_{u,i} - \hat{\mu} - \hat{b_i(t)}\right)
$$
...
The RMSE achieved is typical of models that use similar techniques as ours [Irizarry, Koren]. However, a divergence of ~0.86 stars may be limiting in practice. As per [Koren], we could explore other effects that could yield extra RMSE gains, such as evaluating whether there is a temporal effect on the user behavior, and also explore whether there are item to item, and user to user effects. Additionally, we could add a term that utilizes singular value decompostion (SVD) to surface effects that are harder to explain. These techniques have been shown to significantly increase accuracy, but they also add to the runtime costs of the model, especially in the case of SVD. These conflicting goals would need to be taken into consideration whether a production level solution is needed.
